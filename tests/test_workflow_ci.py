...
def test_finalize_dev_update_uploads_carried_forward_snapshot_without_fresh_files(
    ...
    monkeypatch.setattr(
        finalize_dev_update, "get_latest_snapshot_prefix", lambda client, bucket: "2026-04-09"
    )
    monkeypatch.setattr(
        finalize_dev_update,
        "download_snapshot",
...
def test_finalize_dev_update_cleans_up_temporary_staging_directory(
    ...
    monkeypatch.setattr(finalize_dev_update.tempfile, "TemporaryDirectory", FakeTemporaryDirectory)
    monkeypatch.setattr(
        finalize_dev_update,
        "upload_snapshots",
